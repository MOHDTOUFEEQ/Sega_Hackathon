...
class Oposum {
	constructor({ x, y, width, height, velocity = { x: OPOSUM_X_VELOCITY, y: 0 } }, turningDistance = 100) {
		this.x = x;
		this.y = y;
		this.width = width;
		this.height = height;
		...
		this.deathEffect.src = "/images/enemy-death.png";
	...
	draw(c) {
		...
		if (this.isImageLoaded === true) {
			...
			c.drawImage(this.image, this.currentSprite.x + this.currentSprite.width * this.currentFrame, this.currentSprite.y, this.currentSprite.width, this.currentSprite.height, x, this.y, this.width, this.height);
			c.restore();
		}
	}
	...
	update(deltaTime, collisionBlocks) {
		...
		// Check for platform collisions - safe access to platforms
		if (typeof window.platforms !== "undefined" && window.platforms) {
